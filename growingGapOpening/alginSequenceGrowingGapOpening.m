function [ scores,alignment] = alginSequenceGrowingGapOpening( sequence1 ,sequence2 )
    ...
    for x = 2:s1-1
       for y = 2:s2-1
           ...
           if (gap(x-1,y)~=1)
            score(1,1)=answer(x-1,y)+(gapOpening);
            score(1,2)=answer(x,y-1)+(gapOpening);
   ...
   for x = 2:s1-1
            ...
            if (gap(x-1,y)~=1)
                ...
                score(1,1)=answer(x-1,s2); %no gap penalty as get end of chain
                score(1,2)=answer(x,s2-1)+(gapOpening);
                score(1,3)=getBlosum62Score(sequence1(x-1),sequence2(s2-1))+answer(x-1,s2-1);
            end
            [value,index]=max(score); %get highest
            answer(x,s2)=value; 
   ...
   for y = 2:s2-1
            ...
            if (gap(x-1,y)~=1)
                ...
                score(1,1)=answer(s1-1,y)+gapOpening;
                score(1,2)=answer(s1,y-1);%no gap penalty as get end of chain
                score(1,3)=getBlosum62Score(sequence1(s1-1),sequence2(y-1))+answer(s1-1,y-1);
            end
            [value,index]=max(score);%get highest
   ...
   answer(s1,s2)=value; 
    ...
    for x = s1:-1:1
        for y = s2:-1:1
            toPrint2(x,y)=gap(x,y);
        
        end
    end
   % do trace back on table
    alginment=zeros(3,1);
    alignment=traceBackOpening(answer,sequence1,sequence2,gapPenalty,s1,s2,gap,gapOpening);
